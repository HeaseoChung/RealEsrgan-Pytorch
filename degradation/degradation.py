...
class Degradation:
    def __init__(self, sf=4, shuffle_prob=0.5, isp_model=None):
        self.sf = sf
        self.shuffle_prob = shuffle_prob
        self.isp_model = isp_model

    def analytic_kernel(self, k):
    ...
    def anisotropic_Gaussian(self, ksize=15, theta=np.pi, l1=6, l2=6):
        ...
        k = self.gm_blur_kernel(mean=[0, 0], cov=Sigma, size=ksize)

        return k

    def gm_blur_kernel(self, mean, cov, size=15):
        center = size / 2.0 + 0.5
        ...
        for i in shuffle_order:
            if i == 0:
                ...
                if random.random() < poisson_prob:
                    img = self.add_Poisson_noise(img)
            elif i == 4:
                if random.random() < speckle_prob:
                    img = self.add_speckle_noise(img)
            elif i == 5:
